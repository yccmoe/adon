...
local function ACDP_Init()
	...
	while ( i <=  10) do
		...
		ACDP_Icon[i] = ACDP[i]:CreateTexture("ACDP_Texture", nil, ACDP[i])
...
local function ACDP_OnUpdate(self, elapsed)

	update = update + elapsed

	if update >= 0.25 and #ACDP_SpellList > 0 then
...
ACDP_mainframe:SetScript("OnUpdate",ACDP_OnUpdate)
